...
import {
    ScrollbarGeometry,
    ScrollbarPosition,
} from '@oga/directives/scrollbar/scrollbar.types';
 ...
 */
@Directive({
    selector: '[ogaScrollbar]',
    exportAs: 'ogaScrollbar',
    standalone: true,
})
export class OgaScrollbarDirective implements OnChanges, OnInit, OnDestroy {
    /* eslint-disable @typescript-eslint/naming-convention */
    static ngAcceptInputType_ogaScrollbar: BooleanInput;
    /* eslint-enable @typescript-eslint/naming-convention */

    @Input() ogaScrollbar: boolean = true;
    @Input() ogaScrollbarOptions: PerfectScrollbar.Options;
    ...
    ngOnChanges(changes: SimpleChanges): void {
        // Enabled
        if ('ogaScrollbar' in changes) {
            // Interpret empty string as 'true'
            this.ogaScrollbar = coerceBooleanProperty(
                changes.ogaScrollbar.currentValue
            );

            // If enabled, init the directive
            if (this.ogaScrollbar) {
                this._init();
            }
        ...
        // Scrollbar options
        if ('ogaScrollbarOptions' in changes) {
            // Merge the options
            this._options = merge(
                {},
                this._options,
                changes.ogaScrollbarOptions.currentValue
            );
