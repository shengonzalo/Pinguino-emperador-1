...
import {
    OgaNavigationItem,
    OgaVerticalNavigationAppearance,
    OgaVerticalNavigationMode,
    OgaVerticalNavigationPosition,
} from '@oga/components/navigation/navigation.types';
import { OgaVerticalNavigationAsideItemComponent } from '@oga/components/navigation/vertical/components/aside/aside.component';
...
import { OgaScrollbarDirective } from '@oga/directives/scrollbar/scrollbar.directive';
...
import { OgaHorizontalNavigationBasicItemComponent } from '../horizontal/components/basic/basic.component';
import { OgaHorizontalNavigationBranchItemComponent } from './components/branch/branch.component';
import { TranslocoPipe } from '@ngneat/transloco';
import { TranslocoService } from '@ngneat/transloco';

@Component({
    selector: 'oga-vertical-navigation',
    templateUrl: './vertical.component.html',
    styleUrls: ['./vertical.component.scss'],
    animations: ogaAnimations,
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    exportAs: 'ogaVerticalNavigation',
    standalone: true,
    imports: [
        OgaScrollbarDirective,
        NgFor,
        NgIf,
        OgaVerticalNavigationAsideItemComponent,
...
export class OgaVerticalNavigationComponent
    implements OnChanges, OnInit, AfterViewInit, OnDestroy
{
    ...
    @Input() appearance: OgaVerticalNavigationAppearance = 'default';
    @Input() autoCollapse: boolean = true;
    @Input() inner: boolean = false;
    @Input() mode: OgaVerticalNavigationMode = 'side';
    @Input() name: string = this._ogaUtilsService.randomId();
    @Input() navigation: OgaNavigationItem[];
    @Input() opened: boolean = true;
    @Input() position: OgaVerticalNavigationPosition = 'left';
    @Input() transparentOverlay: boolean = false;
    @Output()
    readonly appearanceChanged: EventEmitter<OgaVerticalNavigationAppearance> =
        new EventEmitter<OgaVerticalNavigationAppearance>();
    @Output() readonly modeChanged: EventEmitter<OgaVerticalNavigationMode> =
        new EventEmitter<OgaVerticalNavigationMode>();
    @Output() readonly openedChanged: EventEmitter<boolean> =
        new EventEmitter<boolean>();
    @Output()
    readonly positionChanged: EventEmitter<OgaVerticalNavigationPosition> =
        new EventEmitter<OgaVerticalNavigationPosition>();
    @ViewChild('navigationContent') private _navigationContentEl: ElementRef;

    activeAsideItemId: string | null = null;
    onCollapsableItemCollapsed: ReplaySubject<OgaNavigationItem> =
        new ReplaySubject<OgaNavigationItem>(1);
    onCollapsableItemExpanded: ReplaySubject<OgaNavigationItem> =
        new ReplaySubject<OgaNavigationItem>(1);
    ...
    private _ogaScrollbarDirectives!: QueryList<OgaScrollbarDirective>;
    private _ogaScrollbarDirectivesSubscription: Subscription;
    ...
    @HostBinding('class') get classList(): any {
        ...
        return {
            ...
            [`oga-vertical-navigation-appearance-${this.appearance}`]: true,
            'oga-vertical-navigation-hover': this._hovered,
            'oga-vertical-navigation-inner': this.inner,
            'oga-vertical-navigation-mode-over': this.mode === 'over',
            'oga-vertical-navigation-mode-side': this.mode === 'side',
            'oga-vertical-navigation-opened': this.opened,
            'oga-vertical-navigation-position-left': this.position === 'left',
            'oga-vertical-navigation-position-right': this.position === 'right',
        };
        /* eslint-enable @typescript-eslint/naming-convention */
    }
    ...
    /**
     * Setter for ogaScrollbarDirectives
     */
    @ViewChildren(OgaScrollbarDirective)
    set ogaScrollbarDirectives(
        ogaScrollbarDirectives: QueryList<OgaScrollbarDirective>
    ) {
        // Store the directives
        this._ogaScrollbarDirectives = ogaScrollbarDirectives;

        // Return if there are no directives
        if (ogaScrollbarDirectives.length === 0) {
            return;
        }

        // Unsubscribe the previous subscriptions
        if (this._ogaScrollbarDirectivesSubscription) {
            this._ogaScrollbarDirectivesSubscription.unsubscribe();
        }

        // Update the scrollbars on collapsable items' collapse/expand
        this._ogaScrollbarDirectivesSubscription = merge(
            ...
            .subscribe(() => {
                ...
                ogaScrollbarDirectives.forEach((ogaScrollbarDirective) => {
                    ogaScrollbarDirective.update();
                });
            });
    }
    ...
    ngOnInit(): void {
        ...
        // Set tooltips for navigation items if appearance is 'thin'
        if (this.appearance === 'thin') {
            this.navigation.forEach((item) => {
                item.tooltip = this._translocoService.translate(
    ...
    ngAfterViewInit(): void {
        ...
        setTimeout(() => {
            ...
            // Otherwise
                ...
                this._ogaScrollbarDirectives.forEach(
                    (ogaScrollbarDirective) => {
                        // Skip if not enabled
                        if (!ogaScrollbarDirective.isEnabled()) {
                            return;
                        }

                        // Scroll to the active element
                        ogaScrollbarDirective.scrollToElement(
                            '.oga-vertical-navigation-item-active',
                            -120,
                            true
                        );
     ...
     * @param item
     */
    openAside(item: OgaNavigationItem): void {
     ...
     * @param item
     */
    toggleAside(item: OgaNavigationItem): void {
        // Toggle
    ...
    onKeyDown(event: KeyboardEvent, item: OgaNavigationItem): void {
        if (event.key === 'Enter' || event.key === ' ') {
            this.toggleAside(item);
        }
    }
    ...
    private _showOverlay(): void {
        ...
        this._overlay.classList.add('oga-vertical-navigation-overlay');

        // Add a class depending on the transparentOverlay option
        if (this.transparentOverlay) {
            this._overlay.classList.add(
                'oga-vertical-navigation-overlay-transparent'
            );
        }
